...
pub struct CreateCommand {
    ...
    /// Address where your InfluxDB server is running, in the form of `<scheme>://<hostname>:<port>`.
    /// At least the port must be provided. The default scheme is `tcp` and the default address is `127.0.0.1`.
    #[arg(long, display_order = 900, id = "SOCKET_ADDRESS", value_parser = SchemeHostnamePort::from_str)]
