...
impl NodeManager {
    ...
    pub async fn create(
        ...
        let mut s = Self {
            ...
            udp_transport: transport_options.udp.as_ref().map(|u| u.transport.clone()),
            secure_channels,
...
#[derive(Debug)]
pub struct NodeManagerTransportOptionsS<T> {
    flow_control_id: FlowControlId,
    transport: T,
}

impl<T> NodeManagerTransportOptionsS<T> {
...
pub struct NodeManagerTransportOptions {
    tcp: NodeManagerTransportOptionsS<TcpTransport>,
    udp: Option<NodeManagerTransportOptionsS<UdpTransport>>,
}
...
impl NodeManagerTransportOptions {
    ...
    pub fn new_tcp(flow_control_id: FlowControlId, transport: TcpTransport) -> Self {
        Self {
            tcp: NodeManagerTransportOptionsS::new(flow_control_id, transport),
            udp: None,
        }
    }
}
