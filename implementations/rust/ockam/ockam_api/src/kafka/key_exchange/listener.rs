...
pub(crate) const KAFKA_KEY_EXCHANGE_LISTENER: &str = "kafka_key_exchange_listener";
        ...
        context
            .send(
                ...
                KeyExchangeResponse {
                    ...
                    secret_key,
                    valid_until: TimestampInSeconds::from(0),
                    rotate_after: TimestampInSeconds::from(0),
                    rekey_period: Duration::from_secs(0),
                },
            )
            .await?;
...
impl KafkaKeyExchangeListener {
    pub async fn create(
        ...
        let address = Address::from_string(KAFKA_KEY_EXCHANGE_LISTENER);
        context
            .flow_controls()
            .add_consumer(address.clone(), secure_channel_flow_control);

        WorkerBuilder::new(KafkaKeyExchangeListener { encryption_at_rest })
