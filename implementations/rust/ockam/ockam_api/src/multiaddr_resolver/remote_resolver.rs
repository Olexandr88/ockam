...
use ockam::udp::{UdpBind, UdpBindArguments, UdpBindOptions, UdpTransport, UDP};
...
impl RemoteMultiaddrResolver {
    ...
    async fn connect_tcp(&self, ma: &MultiAddr, peer: String) -> Result<TcpConnection> {
        let tcp = self
            .tcp
            .as_ref()
            .ok_or_else(|| invalid_multiaddr_error(ma))?;
    ...
    async fn connect_udp(&self, ma: &MultiAddr, peer: &str) -> Result<UdpBind> {
        let udp = self
            .udp
            .as_ref()
            .ok_or_else(|| invalid_multiaddr_error(ma))?;
    ...
    async fn connect(
        ...
        let (transport_type, port) = self.parse_port(ma, it)?;

        let peer = format!("{}:{}", peer, port);

        if transport_type == TCP {
            let connection = self.connect_tcp(ma, peer).await?;
            return Ok(RemoteMultiaddrResolverConnection::Tcp(connection));
        }

        if transport_type == UDP {
            let connection = self.connect_udp(ma, &peer).await?;
    ...
    fn parse_port(&self, ma: &MultiAddr, it: &mut ProtoIter) -> Result<(TransportType, u16)> {
